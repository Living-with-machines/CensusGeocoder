...
def gb1900_compare(census, gb1900, gb1900_candidate_links):
    """Performs fuzzy string matching between candidate links identified between gb1900 and
    census. Selects best batch using a combination of fuzzy string matching score and
    tf-idf weighting. Candidate links with more than one strong match are stored in a
    duplicates table. Returns matches and duplicate matches.
    ...
    pandas.Dataframe
        A pandas dataframe of duplicate matches between gb1900 and census (where there
        are more than 2 or more matches of high quality and there needs to be
        disambiguation)
    ...
    if gb1900_candidate_links.empty:
        ...
        gb1900_comparison_results = gb1900_comparison.compute(
            gb1900_candidate_links, census, gb1900
        )
        gb1900_comparison_results = gb1900_comparison_results.sort_index()
        gb1900_comparison_results = gb1900_comparison_results[
            gb1900_comparison_results["rfuzz_score"] >= 90
        ].copy()
        ...
        if gb1900_comparison_results.empty:
            ...
            gb1900_census_roads_output = pd.merge(
                census,
                gb1900_comparison_results,
                left_index=True,
                right_on="unique_add_id",
            )
            cols_to_use = gb1900_census_roads_output.columns.difference(gb1900.columns)
            gb1900_census_roads_output = pd.merge(
                gb1900,
                gb1900_census_roads_output[cols_to_use],
                left_index=True,
                right_on="pin_id",
            )
            gb1900_census_roads_output = gb1900_census_roads_output.reset_index()
            gb1900_census_roads_output = gb1900_census_roads_output.sort_values(
                by="unique_add_id"
            )

            gb1900_census_roads_output["rfuzz_weighted"] = (
                gb1900_census_roads_output["rfuzz_score"]
                * gb1900_census_roads_output["weighting"]
            )
            gb1900_census_roads_output_maxonly = gb1900_census_roads_output[
                gb1900_census_roads_output["rfuzz_weighted"]
                == gb1900_census_roads_output.groupby("unique_add_id")[
                    "rfuzz_weighted"
                ].transform("max")
            ]
            # print(len(gb1900_census_roads_output_maxonly))
            gb1900_census_roads_output_filtered_deduplicated = gb1900_census_roads_output_maxonly.drop_duplicates(
                subset=["unique_add_id"], keep=False
            )
            # print(gb1900_census_roads_output_filtered_deduplicated)
            gb1900_census_roads_output_filtered_duplicates = gb1900_census_roads_output_maxonly[
                gb1900_census_roads_output_maxonly.duplicated(
                    subset=["unique_add_id"], keep=False
                )
            ]
    ...
    return (
        gb1900_census_roads_output_filtered_deduplicated,
        gb1900_census_roads_output_filtered_duplicates,
    )
...
def compare(census, os, os_candidate_links, new_uid, geom_attributes, census_fields):
    ...
    if os_candidate_links.empty:
        ...
        os_comparison_results = os_comparison_results[
            os_comparison_results["rfuzz_score"] >= 90
        ].copy()
        ...
        if os_comparison_results.empty:
            ...
            os_census_roads_output = pd.merge(
                census, os_comparison_results, left_index=True, right_on="unique_add_id"
            ).reset_index()
            ...
            os_census_roads_output["rfuzz_weighted"] = (
                os_census_roads_output["rfuzz_score"]
                * os_census_roads_output["weighting"]
            )
            os_census_roads_output_maxonly = os_census_roads_output[
                os_census_roads_output["rfuzz_weighted"]
                == os_census_roads_output.groupby("unique_add_id")[
                    "rfuzz_weighted"
                ].transform("max")
            ]
