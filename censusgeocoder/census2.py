...
class Census_vars():
    ...
    def __post_init__(self):
        self.census_read_library = utils.get_readlibrary(self.census_file, self.read_csv_params)
        utils.validate_pandas_read_csv_kwargs(self.census_file, self.read_csv_params,)
        ...
        if self.process != False:
            ...
            if self.lkup_file != None:
                self.census_lkup_read_library = utils.get_readlibrary(self.lkup_file, self.lkup_params, )
...
class Census():
    ...
    def process_census(self, ):
        ...
        if self.vars.field_to_clean != None:
            self.data, field_to_clean_new = utils.clean_address_data(self.data, 
        ...
        self._write_census_data("standardised", self.vars.write_processed_csv_params, )
    ...
    def read_census(self, ):
        ...
        self.data = self.vars.census_read_library(self.vars.census_file,  
                                        **self.vars.read_csv_params,)
        

    def _gensubsetlist(self, ):
    ...
    def _write_census_data(self, status, params, ):
        ...
        if type(self.vars.subsetlist) == np.ndarray:
            for sub in self.vars.subsetlist:
                ...
                output_path_components = [str(x) for x in [self.vars.output_path, self.vars.country, self.vars.year, sub, filename]]
            ...
            output_path_components = [str(x) for x in [self.vars.output_path, self.vars.country, self.vars.year, filename]]
    ...
    def add_lkup(self, ):
        if self.vars.lkup_file != None:
            lkup_data = self.vars.census_lkup_read_library(self.vars.lkup_file, 
    ...
    def create_uid_of_geocode_field(self, ):
        ...
        groupby_cols = []
        groupby_cols.extend([x for x in utils.flatten(self.vars.boundaries_field)]) # this may not be correct as the boundaries field should include ConParID and CEN_1851
        groupby_cols.append(self.vars.field_to_geocode)


        self.data[self.vars.unique_field_to_geocode_name] = self.data.groupby(groupby_cols,
                                                                              dropna=False).ngroup()


        self.data = self.data.dropna(subset=self.vars.unique_field_to_geocode_name).copy()

        self.data[self.vars.unique_field_to_geocode_name] = pd.to_numeric(self.data[self.vars.unique_field_to_geocode_name], 
                                                                          downcast="integer")

        self._write_census_data("address_uid", self.vars.write_processed_csv_params, )

    def create_censusforlinking(self, ):
        self.data = self.data.drop_duplicates(subset = [self.vars.unique_field_to_geocode_name], )
        ...
        # self.data = self.data[]
        self._write_census_data("slim", self.vars.write_processed_csv_params_slim, )
    ...
    def geocode(self,
        ...
        if type(self.vars.subsetlist) != np.ndarray:
            ...
            geocoded = geocode.GeoCode(census_data=self.data,
            ...
            for x, y in geocoded.rslts_dict.items():
                ...
                utils.write_df_to_file(y, output_path_components, self.vars.write_processed_csv_params, )

        else:

            for cen in self.vars.subsetlist:
                census_data = self.data[self.data[self.vars.subset_field] == cen]
                geocoded = geocode.GeoCode(census_data=census_data,
                            census_geocode_field=self.vars.field_to_geocode,
                            census_indexfield=self.vars.unique_field_to_geocode_name, 
                            target_geometry_data=target_geometry.data,
                ...
                for x, y in geocoded.rslts_dict.items():
                    ...
                    output_path_components = [str(x) for x in [self.vars.output_path, self.vars.country, self.vars.year, cen, filename]]
