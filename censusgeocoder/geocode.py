...
class GeoCode:
    ...
    def compare(
        ...
        """
        Performs fuzzy string matching between candidate links identified by create_candidate_links().
    ...
    def process_results(
        ...
        linked_duplicates: pandas.Dataframe
            ...
            (where there is >1 match of equal, high quality).

        linked_leftovers: pandas.DataFrame
            A pandas DataFrame of comparison results meeting threshold criteria but not included in linked or linked_duplicates.
        """
        ...
        if target_results.empty:
            ...
            linked_all_maxonly = linked_all[
                linked_all[self.final_score_field]
                == linked_all.groupby(self.census_data.index.name)[
            ...
            linked_leftovers = linked_all[
                (linked_all.index.isin(linked.index) == False)
    ...
    def _calc_finalscore(self, linked_all):
        if self.comparison_method == "1911_bespoke":
            linked_all[list(self.comparers.values())[2]] = linked_all.groupby(
                self.census_data.index.name
            )[list(self.comparers.values())[2]].transform("rank", ascending=False)

        linked_all[self.final_score_field] = linked_all[
